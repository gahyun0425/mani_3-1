...
public:
      ...
      first_plan_done_(false)
    {
        ...
        start_sub_ = this->create_subscription<std_msgs::msg::Bool>(
            "/start_command", 10,
            std::bind(&RSNode::startCallback, this, std::placeholders::_1)
        );
...
private:
    ...
    void startCallback(const std_msgs::msg::Bool::SharedPtr msg) {
        if (!has_goal_been_set_) {
            RCLCPP_WARN(this->get_logger(), "[RSNode] goal 좌표가 설정되지 않아 시작할 수 없습니다.");
            return;
        }
        if (msg->data) {
    ...
    void goalCallback(const std_msgs::msg::Float64MultiArray::SharedPtr msg) {
        if (msg->data.size() != 3) {
            RCLCPP_ERROR(this->get_logger(), "[RSNode] goal 메시지 크기 오류: size=%zu", msg->data.size());
            return;
        }

        // 이전 goal이 출발점이 됨
        if (has_goal_been_set_) {
            start_ = prev_goal_point_;
        } else {
            start_ = makePoint(0.14, 0.0, 1.005);  // 초기 고정 위치
        }

        goal_ = makePoint(msg->data[0], msg->data[1], msg->data[2]);
        prev_goal_point_ = goal_;
        has_goal_been_set_ = true;

        RCLCPP_INFO(this->get_logger(),
            "[RSNode] Goal 설정됨: start=(%.3f, %.3f, %.3f), goal=(%.3f, %.3f, %.3f)",
